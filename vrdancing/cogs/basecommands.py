...
class basecommands(commands.Cog):
    ...
    async def SetMyName(self, ctx, name: str):
        ...
        if (len(name) < config.MINLENUSERNAME):
            await ctx.reply(f"Username too short. Name must be between {config.MINLENUSERNAME} and {config.MAXLENUSERNAME} letters.", mention_author=True)
            return
        if (len(name) > config.MAXLENUSERNAME):
            await ctx.reply(f"Username too long. Name must be between {config.MINLENUSERNAME} and {config.MAXLENUSERNAME} letters.", mention_author=True)
            return
    ...
    async def SetDesc(self, ctx, desc: str):
        ...
        if len(desc) < config.MINLENDESC:
            await ctx.reply(f"Description too short. Character amount must lie between {config.MINLENDESC} and {config.MAXLENDESC}! Also don't forget to put everything into ''!")
            return;
        if len(desc) > config.MAXLENDESC:
            await ctx.reply(f"Description too long. Character amount must lie between {config.MINLENDESC} and {config.MAXLENDESC}!")
            return;

        config.Glogger.Log(f"{ctx.author.name} updated there description")
        await SetDesc(desc, ctx.author)
        await ctx.reply(f"```Description sucessfully changed. You can check by using the '{config.PREFIX}whois' command without any arguments!```", mention_author=True)
