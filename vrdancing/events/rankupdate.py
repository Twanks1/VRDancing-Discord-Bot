...
async def AddSWSXP(usr: str, ctx):
    ...
    await config.db.execute(
        "UPDATE ranks SET swsxp = $1 WHERE discordid = $2", True, row["discordid"]
    )
...
async def onRankChange(ctx, usr: str, rank: str):
    ...
    try:
        ...
        config.Glogger.Warn(
            f"Couldn't remove role {row['rank']} from user {usr} (Does the role exist?)"
        )
    ...
    try:
        ...
        config.Glogger.Warn(
            f"Couldn't add role {row['rank']} to user {usr} (Does the role exist?)"
        )
    ...
    try:
        ...
        card = await GenerateRankCard(user)
        await channel.send(
            f"{user.mention} just achieved a new rank: {rank} ({config.gRanks[newRankIndex].requiredPoints} XP was needed)",
            file=card,
        )
    except:
        config.Glogger.Warn(
            "Failed to send a rank up message in the rank up channel (Wrong channel id set?)"
        )
        pass
